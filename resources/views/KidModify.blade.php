...
<div class="container w-25">
    <form action="/kid/{{ $kid->id }}/update" method="POST">
        @method('PUT')
        @csrf
        <div class="row my-1">
            <label for="name" class="col">Nom</label>
            <input readonly type="text" name="name" id="name" value="{{ $kid->name }}" class="col bg-light">
        </div>
        <div class="row my-1">
            <label for="firstName" class="col">Prenom</label>
            <input readonly type="text" name="firstName" id="firstName" value="{{ $kid->firstName }}" class="col bg-light">
        </div>
        <div class="row my-1">
            <label for="birthdate" class="col">Date de naissance</label>
            <input readonly type="date" name="birthdate" id="birthdate" value="{{ $kid->birthdate }}" class="col bg-light">
        </div>
        <div class="row my-1">
            <label for="address" class="col">Adresse</label>
            <input type="text" name="address" id="address" value="{{ $kid->address }}" class="col">
        </div>
        <div class="row my-1">
            <label for="city" class="col">Ville</label>
            <input type="text" name="city" id="city" value="{{ $kid->city }}" class="col">
        </div>
        <div class="row my-1">
            <label for="state" class="col">Province</label>
            <select name="state_id" id="state" class="col">
                @foreach($states as $state)
                <option value="{{ $state->id }}" {{ $state->id == $kid->state_id ? 'selected' : '' }}>
                    {{ $state->description }}
                </option>
                @endforeach
        ...
        <div class="row my-1">
            ...
            <input type="text" pattern="{0 - 9}" name="phone" id="phone" value="{{ $kid->phone }}" class="col">
        </div>
        <div class="my-3">
